  ...
  //eingelade User Hinzufuegen
    ...
    var existUser = "SELECT * from User where User.name = ?";
		var retValue = existValueInDatabase(existUser, [user.email], function (exists) {
		  if (!exists) {
			  log('debug','add user ' + user.email + ', ' + user.uid + ' to DB');
				log('debug','add user ' + existUser + ' and ' + exists);
        //userID-null | name? | -pwd- | -considered- | SSO | FullName | considerationString | salt | active | uid  
        //benoetigen: name (email), FullName, uid 
				var addUserSql = "INSERT INTO User VALUES(null, ?, 0, 0, ?, ?, 0, 0, 1, ?)";
				var addUserQuery = connection.query(addUserSql, [user.email, null, user.displayName, user.uid]);
  ...
  var samlStrategy = new saml.Strategy({
    ...
    issuer: 'https://etherpad.rrze.uni-erlangen.de/metadata',    //issuer string to supply to identity provider
      ...
      registerUser(user, function(userAdded, finished){
        log('debug', '@hooks.js:453 finished:  ' +  finished);
				if(userAdded){
