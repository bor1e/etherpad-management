...
// --------------------- packages for SSO-SAML ---------------------------------
var passport = require('passport');
var SamlStrategy = require('passport-saml/lib/passport-saml/index.js').Strategy;
...
var userAuthenticated = function (req, cb) {
    log('debug', 'userAuthenticated \t' +req.session.username +'\t'+ req.session.userId +'\t'+ req.session.email );
    if (req.session.username && req.session.userId && req.session.email) {
        cb(true);
    } else {
        cb(false);
    }
...
var userAuthentication = function (username, cb) {
...
//    log('debug', 'existValueInDatabase ' + params);
...
function updateSql(sqlUpdate, params, cb) {
    log('debug', 'updateSql' + sqlUpdate + ' ' + params + ' ' + cb);
...
//Finde User mit Hilfe seiner uid-Kennung
function getUserUid(uid, cb) {
    log('debug', 'getUserUid');
    var sql = "Select * from User where uid = ?";
    getOneValueSql(sql, [uid], cb);
}
...
exports.expressCreateServer = function (hook_name, args, cb) {
  //eingelade User Hinzufuegen
	function notRegisteredUpdate(userid, groupid, email) {
    //Values sind UserID, GroupId und seine Rechte. 1 fuer Owner, 2 fuer eingeladen etc.
    ...
    var existUser = "SELECT * from User where User.name = ?";
		...
		log('error', 'padtest:392:hooks\t'+user.email);
		  if (exists) {
		log('error', 'padtest:394:hooks user:\t'+user.email+'\t exists');
        cb(USER_EXISTS);
      }
      else {
        //userID-null | emaile | -considered- | FullName | considerationString | active | uid
				//var addUserSql = "INSERT INTO User VALUES(null, ?, 0, 0, 0, ?, 0, 0, 1, ?)";
				var addUserSql = "INSERT INTO User VALUES(null, ?, 0, ?, 0, 1, ?)";
				var addUserQuery = connection.query(addUserSql, [user.email, user.FullName, user.uid]);
				log('debug','add user ' + user.uid);
              ...
              checkInvitations(user.email, newUser.insertId, function(){
						...
						addUserQuery.on('end', function () {
						  cb('User ' + user.FullName + ' successfully added');
						});
      }
			return exists;
    });
  ...
  var samlStrategy = new saml.Strategy({
    callbackUrl: 'https://pad.rrze.fau.de/pass/login/callback',
    entryPoint: 'https://www.sso.uni-erlangen.de/simplesaml/saml2/idp/SSOService.php',
    issuer: 'https://pad.rrze.fau.de/metadata',    //issuer string to supply to identity provider
    cert: fs.readFileSync('/opt/etherpad/etherpad-lite/node_modules/ep_user_pad_frontend/idp_cert.crt', 'utf8'),
    }, function(profile, done) {
      var user = {};
      user.uid = profile['urn:mace:dir:attribute-def:uid'];
      user.email = profile['urn:mace:dir:attribute-def:mail'];
      user.FullName = profile['urn:mace:dir:attribute-def:displayName'];
      user.username = profile['urn:mace:dir:attribute-def:givenName'];
     // user.name = profile['urn:mace:dir:attribute-def:sn']
      registerUser(user, function(msg){
  ...
  args.app.use('/pass', router);

  router.use(bodyParser.urlencoded({extended: false}));
  router.use(bodyParser.json());
  router.use(morgan('combined'));
  router.use(cookieParser('flavour'));
  router.use(methodOverride());
  router.use(expressSession({ secret: 'missionAccomplished', saveUninitialized: false, resave: false}));
  router.use(passport.initialize());
  router.use(passport.session());

  //router.get('/', function(req, res){
  //  res.send('hey there folks =)');
  //  console.log('I am here, Roger?!');
  //});

  router.get('/login',
    passport.authenticate('saml', {failureRedirect: '/', failureFlash: true, samlFallback: 'login-request'}),
		    function(req, res) {
				  res.redirect('/');
			  }
  );
  ...
  router.post('/login/callback',
    passport.authenticate('saml', {failureRedirect:'/', failureFlash: true}),
      function(req, res){
        getUserUid(req.user.uid, function(found, currUser){
	log('error','padtest not found: \t' + req.user.uid);
          if(found){
            req.session.userId  = currUser[0].userID;
            req.session.email   = req.user.email ;
            req.session.uid     = req.user.uid ;
            req.session.username= req.user.username ;
            req.session.baseurl = 'https://pad.rrze.fau.de/';
           	res.redirect('/');
						return true;
					}	else {
  ...
  router.get('/logout', function (req, res) {
    req.logout();
    //req.session.reload(function(reg, res){
    //session update
    //});
    req.session.destroy();
  ...
  args.app.get('/metadata', function(req, res){
    res.redirect('/pass/metadata');
  });

  args.app.get('/login', function(req, res){
    res.redirect('/pass/login');
  });
  args.app.get('/impressum', function (req, res) {
    var render_args = {
      errors: []
    };
    res.send(eejs.require("ep_user_pad_frontend/bootstrap/impressum.ejs", render_args));
  });
/*
args.app.get('/impressum', function(req, res){
    res.redirect('https://www.rrze.fau.de/impressum.shtml');
  });
*/
  args.app.get('/logout', function(req, res){
    res.redirect('/pass/logout');
  });

  args.app.get('/imprint.html', function(req, res){
        res.send(eejs.require("ep_user_pad_frontend/templates/impressum.ejs"));
    //res.sendFile("./templates/imprint.html", {root: __dirname});
  });

  args.app.get('/index', function (req, res) {
	  userAuthenticated(req, function (authenticated) {
        ...
        res.send(eejs.require("ep_user_pad_frontend/bootstrap/index.ejs", render_args));
      }
    });
  });
  ...
  args.app.get('/index.html', function (req, res) {
	  userAuthenticated(req, function (authenticated) {
        ...
        res.send(eejs.require("ep_user_pad_frontend/templates/index.ejs", render_args));
      }
    });
  });
        ...
        getUser(req.session.userId, function (found, currUser) {
          var render_args;
            if (currUser && currUser.length > 0) {
              ...
              res.send(eejs.require("ep_user_pad_frontend/templates/help.ejs", render_args));
            } else {
              render_args = {
                errors: [],
                username: ""
...
/*
  args.app.get('/homecss.html', function (req, res) {
    userAuthenticated(req, function (authenticated) {
      if (authenticated) {
        getUser(req.session.userId, function (found, currUser) {
          var render_args;
            if (currUser && currUser.length > 0) {
              ...
              res.send(eejs.require("ep_user_pad_frontend/templates/homecss.ejs", render_args));
            } else {
              render_args = {
                errors: [],
                username: ""
...
*/
        ...
        getAllSql(sql, [req.session.userId], function (groups) {
          getUser(req.session.userId, function (found, currUser) {
            ...
            if (currUser) {
              ...
              res.send(eejs.require("ep_user_pad_frontend/templates/home.ejs", render_args));
            } else {
              render_args = {
                errors: [],
                username: " ",
        ...
        getAllSql(sql, [req.session.userId], function (groups) {
          getUser(req.session.userId, function (found, currUser) {
            ...
            if (currUser) {
              ...
              res.send(eejs.require("ep_user_pad_frontend/bootstrap/home.ejs", render_args));
            } else {
              render_args = {
                errors: [],
                username: " ",
      ...
      if (authenticated) {
        getUser(req.session.userId, function (found, currUser) {
          if (currUser) {
            ...
            res.send(eejs.require("ep_user_pad_frontend/templates/pads.ejs", render_args));
          }
        });
      } else {
        res.redirect("../index.html");
  ...
  args.app.get('/group-data.html/:groupid', function (req, res) {
    log('debug', req.path);
    userAuthenticated(req, function (authenticated) {
    settings.globalUserName=req.session.username;
            ...
            getGroup(req.params.groupid, function (found, currGroup) {
              ...
              getUserGroup(req.params.groupid, req.session.userId, function (found, currUserGroup) {
                ...
                if (!currUserGroup) {
                  ...
                  res.send(eejs.require("ep_user_pad_frontend/templates/msgtemplate.ejs", render_args));
                  return;
                }
                ...
                if (currGroup && currUser && currUserGroup) {
                  ...
                  res.send(eejs.require("ep_user_pad_frontend/templates/group-data.ejs", render_args));
                } else {
                  render_args = {
                    errors: [],
            ...
            getGroup(req.params.groupid, function (found, currGroup) {
              getUserGroup(req.params.groupid, req.session.userId, function (found, currUserGroup) {
                ...
                if (currGroup && currUser && currUserGroup) {
                  ...
                  res.send(eejs.require("ep_user_pad_frontend/bootstrap/group.ejs", render_args));
                } else {
                  render_args = {
                    errors: [],
                    msg: "This group does not exist! Perhabs anyone has deleted the group."
  ...
  args.app.get('/groups', function (req, res) {
    userAuthenticated(req, function (authenticated) {
      if (authenticated) {
        ...
        getAllSql(sql, [req.session.userId], function (groups) {
          getUser(req.session.userId, function (found, currUser) {
            ...
            res.send(eejs.require("ep_user_pad_frontend/bootstrap/groups.ejs", render_args));
          });
        });
      } else {
        res.redirect("../index.html");
            ...
            if (!(userGroup[0].Role == 1)) {
              ...
              getUser(req.session.userId, function (found, currUser) {
                for (var i = 0; i < fields.users.length; i++) {
                  if (fields.users[i] != "") {
                    var userEmail = fields.users[i].toString().toLowerCase().replace(/\s/g, '');
                    log('error', 'inviteUsers: \nuserEmail:' + userEmail + ' \nfields.locaton:'+ fields.location + ' \nfields.groupID'+fields.groupID + '\nres:' + res  + '\ncurruser[0].Fullname:' + currUser[0].FullName);
  ...
  //TODO do we want to be able to change the usernames
            ...
            if (found) {
              getEtherpadGroupFromNormalGroup(fields.groupId, function (group) {
                addUserToEtherpad(req.session.userId, function (etherpad_author) {
                  sessionManager.createSession(group, etherpad_author.authorID, Date.now() + 7200000, function (err, session) {
                    var data = {};
                    data.success = true;
                    ...
                    //log(data);
                    //console.error('session: %s,\n group: %s,\n pad_name: %s,\n location: %s', data.session, data.group, data.pad_name, data.location);
                    res.send(data);
                  });
                });
              });
              ...
              if (currGroup && currGroup.length > 0) {
                if (authenticated) {
                  if (currUser) {
                    ...
                    //console.log('IMPORTANT PADURL: ' + render_args.padurl + '\n baseurl: ' + req.session.baseurl );
                    res.send(eejs.require("ep_user_pad_frontend/templates/pad.ejs",	render_args));
                  } else
                    res.send("Error");
                } else {
                  render_args = {
                  ...
                  res.send(eejs.require("ep_user_pad_frontend/templates/pad_with_login.ejs", render_args));
                }
              } else {
                render_args = {
                  errors: [],
  ...
  args.app.get('/public_pad/:id', function (req, res) {
    userAuthenticated(req, function (authenticated) {
      ...
      if (authenticated) {
        ...
        res.send(eejs.require("ep_user_pad_frontend/templates/public_pad_logged_in.ejs", render_args));
      } else {
        render_args = {
          errors: [],
          padurl: "./../p/" + req.params.id
        };
        res.send(eejs.require("ep_user_pad_frontend/templates/public_pad.ejs", render_args));
      }
    });
  });
...
exports.eejsBlock_indexWrapper = function (hook_name, args, cb) {
  args.content = eejs.require("ep_user_pad_frontend/templates/index_redirect.ejs");
  return cb();
};
